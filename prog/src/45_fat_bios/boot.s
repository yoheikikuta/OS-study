...
read_file:
    ...
    ;-----------------------
    ; Read sector of root directory
    ;-----------------------
    mov    bx, 32 + 256 + 256  ; BX = (head sector of directory entry)
    mov    cx, (512 * 32) / 512  ; CX = (sector number corresponding to 512 entries)
.10L:

    ;-----------------------
    ; Read one sector (16 entries)
    ;-----------------------
    ...
    ;-----------------------
    cdecl  fat_find_file   ; AX = (search file)
    cmp    ax, 0           ; if (AX)
    je     .12E

    add    ax, 32 + 256 + 256 + 32 - 2  ; Add offset to sector position
    cdecl  read_lba, BOOT, ax, 1, 0x7800  ; read_lba()

    jmp    .10E             ; break
.12E:
    inc    bx               ; BX++  Next sector (16 entries)
    loop   .10L
.10E:
...
fat_find_file:
    ...
    ;-----------------------
    cld                    ; DF clear
    mov    bx, 0           ; BX = (file head sector)
    mov    cx, 512 / 32    ; CX = (entry number)
    mov    si, 0x7600      ; SI = (read sector address)

.10L:
    and    [si + 11], byte 0x18  ; SI += 32
    jnz    .12E

    cdecl  memcmp, si, .s0, 8 + 3  ; AX = memcmp(file name)
    cmp    ax, 0           ; if (identical files)
    jne    .12E

    mov    bx, word [si + 0x1A]  ; BX = (head sector of file)
    jmp    .10E
...
.12E:
    ...
    loop   .10L
.10E:
    mov    ax, bx           ; ret = (head sector of found file)
...
;***********************
; Program of moving to real mode
;***********************
TO_REAL_MODE:
    ;-----------------------
...
.bit16:	 mov    ax, 0x0020  ; DS = 0x20 (data segment selector)
    ...
    ;-----------------------
    ; Move to 16 bit real mode
    ;-----------------------
    mov    eax, cr0        ; Clear PG/PE bit
    and    eax, 0x7FFF_FFFE  ; CR0 &= ~(PG|PE)
    mov    cr0, eax
    jmp    $ + 2

    ;-----------------------
    ; Segment setting (real mode)
    ;-----------------------
...
.real:	mov    ax, 0x0000
    ...
    ;-----------------------
    ; Set interruption mask (real mode)
    ;-----------------------
    ...
    outp   0x21, 0b_1011_1000  ; Enable interruption  FDD/Slave/PIC/KBC/timer
    outp   0xA1, 0b_1011_1110  ; Enable interruption  HDD
    ...
    ;-----------------------
    ; Set interruption mask (protected mode)
    ;-----------------------
    cli                    ; Forbid interruption
    ...
    outp   0xA1, 0x28      ; SLAVE.ICW2 = 0x10
    outp   0xA1, 0x02      ; SLAVE.ICW3 = 0x02
    outp   0xA1, 0x01      ; SLAVE.ICW4 = 0x01

    outp   0x21, 0b_1111_1000  ; Enable interruption  FDD/Slave/PIC/KBC/timer
    outp   0xA1, 0b_1111_1110  ; Enable interruption  RTC
    ...
    jmp    $ + 2           ; Clear read instructions in advance
...
.bit32:  mov    ax, 0x0010  ; DS = 32 bit DS
    mov    ds, ax
    mov    es, ax
    mov    ss, ax

    ;-----------------------
    ; Recover register setting
    ;-----------------------
    mov    esp, [.esp_saved]
    mov    eax, [.cr0_saved]
    mov    cr0, eax
